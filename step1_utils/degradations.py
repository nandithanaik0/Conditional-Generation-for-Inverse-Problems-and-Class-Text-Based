...
def get_degradation(conf, device):
    if conf.degradation == "Inpainting":
        import random
        import numpy as np
        def get_inpainting_mask(mask_type):
            mask = np.ones((256, 256), dtype=int)
            if mask_type == 'random':
                random.seed(0)
                random_indices = random.sample(range(256 * 256), int(np.floor(256.0 * 256.0 * conf.random_amount)))
                row_indices, col_indices = np.unravel_index(random_indices, (256, 256))
                mask[row_indices, col_indices] = 0
            elif mask_type == 'box':
                x,y,width,height = conf.box_indices
                mask[x:x+width, y:y+height] = 0
            return torch.tensor(mask)
        loaded = get_inpainting_mask(conf.mask_type).to(device)
        mask = loaded.to(device).reshape(-1)
        missing_r = torch.nonzero(mask == 0).long().reshape(-1) * 3
        missing_g = missing_r + 1
        missing_b = missing_g + 1
        missing = torch.cat([missing_r, missing_g, missing_b], dim=0)
        return Inpainting(3, 256, missing, device)
    
    if conf.degradation == "SR":
        import numpy as np
        factor = conf.scale_factor
        ...
        return SRConv(kernel / kernel.sum(), 3, 256, device, stride=factor)
